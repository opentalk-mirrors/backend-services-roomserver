...
use crate::{
    command::LiveKitCommand, error::LiveKitError, event::LiveKitEvent, loopback::LiveKitLoopback,
};
...
impl LiveKitModule {
    ...
    fn notify_screen_share_permission_update(
        ...
        ctx.send_ws_message(
            [sender],
            event::LiveKitEvent::ScreenSharePermissionsUpdated {
                grant,
    ...
    fn issue_popout_stream_access_token(
        ...
        ctx.send_ws_message(
            [participant_id],
            event::LiveKitEvent::PopoutStreamAccessToken { token },
        )?;
